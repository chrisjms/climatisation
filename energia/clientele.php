...
function sort_clients_alpha(array &$energia_clients): void {
    if (empty($energia_clients)) return;
    if (class_exists('Collator')) {
        $coll = new Collator('fr_FR');
        if (method_exists($coll, 'setStrength')) { $coll->setStrength(Collator::PRIMARY); }
        usort($energia_clients, function($a, $b) use ($coll) {
    ...
    usort($energia_clients, function($a, $b) use ($norm) {
...
function fetch_bdc_for_client(PDO $pdo, int $clientId, bool $debug = false, array &$log = []): array {
    ...
    $candidateTables = [
        'energia_bons_de_commande','bons_commandes','bon_de_commande',
    ...
    if ($colDev && table_has_columns($pdo, 'energia_devis', ['id','client_id'])) {
        $sql = "SELECT ".implode(", ", $select)."
                  FROM `$t` t
                  JOIN `energia_devis` d ON d.`id` = t.`$colDev`
                 WHERE d.`client_id` = :cid
              ORDER BY $orderBy";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':cid' => $clientId]);
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        if ($debug) $log[] = "Résultats via JOIN energia_devis : ".count($rows);
        if ($rows) return $rows;
    }

    if ($colFac && table_has_columns($pdo, 'energia_factures', ['id','client_id'])) {
        $sql = "SELECT ".implode(", ", $select)."
                  FROM `$t` t
                  JOIN `energia_factures` f ON f.`id` = t.`$colFac`
                 WHERE f.`client_id` = :cid
              ORDER BY $orderBy";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':cid' => $clientId]);
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        if ($debug) $log[] = "Résultats via JOIN energia_factures : ".count($rows);
        if ($rows) return $rows;
    }
...
if ($search !== '') {
    ...
    $energia_clients = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
    $energia_clients = $pdo->query(
...
sort_clients_alpha($energia_clients);

/* ──── Préchargement téléphones / e-mails pour la liste ──── */
$phonesByClient = [];
$emailsByClient = [];
if (!empty($energia_clients)) {
    $ids = array_column($energia_clients, 'id');
    $in  = implode(',', array_fill(0, count($ids), '?'));

    $stP = $pdo->prepare("SELECT client_id, phone, label FROM energia_client_telephones WHERE client_id IN ($in) ORDER BY id");
    $stP->execute($ids);
    ...
    $stE = $pdo->prepare("SELECT client_id, email, label FROM energia_client_emails WHERE client_id IN ($in) ORDER BY id");
    $stE->execute($ids);
...
if (table_exists($pdo, 'energia_client_documents')) {
    $docCols = list_columns($pdo, 'energia_client_documents');
...
if ($selected_client_id) {
    $stmt = $pdo->prepare(
		'SELECT id, code_client, nom, prenom, telephone, email, adresse, code_postal, ville, details, date_ajout
		 FROM energia_clients
		 WHERE id = ?'
	);
    ...
    $stmt = $pdo->prepare('SELECT phone, label FROM energia_client_telephones WHERE client_id = ? ORDER BY id');
    $stmt->execute([$selected_client_id]);
    $client_phones = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];

    $stmt = $pdo->prepare('SELECT email, label FROM energia_client_emails WHERE client_id = ? ORDER BY id');
    $stmt->execute([$selected_client_id]);
    $energia_client_emails = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    ...
    $stmt->execute([$selected_client_id]);
    $energia_devis = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $stmt = $pdo->prepare(
        'SELECT id, description, montant_ttc, date_creation, fichier_pdf
           FROM energia_factures
          WHERE client_id = ?
       ORDER BY date_creation DESC'
    );
    $stmt->execute([$selected_client_id]);
    $energia_factures = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
function initials($codeClient = '', $prenom = '', $nom = ''){
    $codeClient = trim((string)$codeClient);
    if ($codeClient !== '') {
        return mb_strtoupper(mb_substr($codeClient, 0, 3));
    }
...
<style>
  ...
  .energia_clients-panel.collapsed .list-hint{display:flex}
  .energia_clients-panel .list-hint{display:none;align-items:center;gap:8px;margin:6px 0 0}
  .energia_clients-panel.collapsed .client-item{display:none}
  .energia_clients-panel.collapsed .client-item.is-selected{display:flex}
...
function filterClients(){
    ...
    document.querySelectorAll(".client-item").forEach(li=>{
        li.style.display=li.classList.contains('is-selected') && document.querySelector('.energia_clients-panel')?.classList.contains('collapsed')
          ? 'flex'
...
<div class="main">
  ...
  <!-- ============== 1) RECHERCHE & LISTE ============== -->
  <section id="recherche" class="card energia_clients-panel <?= $selected_client_id ? 'collapsed' : '' ?>">
    ...
    <ul class="client-list">
        <?php foreach ($energia_clients as $c): ?>
            <?php
                $cid = (int)$c['id'];
  ...
  <?php
    ...
    $allEmails = $energia_client_emails;
  ...
  <section id="fiche" class="card" style="margin-top:16px;">
    ...
    <div class="client-header">
      <div class="left" style="display:flex;gap:10px;align-items:center">
        <div class="avatar"><?= h(initials($client_infos['code_client'] ?? '', $client_infos['prenom'] ?? '', $client_infos['nom'] ?? '')) ?></div>
        <div>
          <div style="font-weight:bold;font-size:16px">
			  <?= h(
				trim(
				  ($client_infos['code_client'] ?? '') . ' ' .
				  ($client_infos['nom'] ?? '') . ' ' .
				  ($client_infos['prenom'] ?? '')
				)
			  ) ?>
...
<div class="small-muted">
            ...
            <span class="badge">🧾 <?= count($energia_devis) ?> energia_devis</span>
            <span class="badge">💶 <?= count($energia_factures) ?> energia_factures</span>
    ...
    <div class="tabs">
      ...
      <button class="tab-btn" data-tab="energia_devis">Devis (<?= count($energia_devis) ?>)</button>
      <button class="tab-btn" data-tab="energia_factures">Factures (<?= count($energia_factures) ?>)</button>
      ...
      <div class="info-grid" id="coordGrid">
        <div class="info-card" id="cardCoord">
          ...
          <div id="coordView" style="margin-top:8px">
			<p><strong>Code client :</strong><br>
			  <?= h($client_infos['code_client'] ?? '—') ?>
			 </p>
  ...
  <!-- Ligne 1 : Code client (pleine largeur) -->
  <div style="grid-column:1 / -1">
    <label>Code client<br>
      <input type="text" name="code_client" maxlength="100"
             value="<?= h($client_infos['code_client'] ?? '') ?>">
    </label>
  </div>
  ...
  <div>
      ...
      <div class="upload-form info-card">
        ...
        <form action="upload_document.php" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
            ...
            <label>Type de document<br><input type="text" name="type" placeholder="photo / energia_devis / entretien" required></label>
          </div>
          ...
          <div class="small-muted" style="margin-top:6px">
            La date ci-dessus (si fournie) sera enregistrée dans <code>energia_client_documents</code> (colonne détectée
    ...
    <div id="pane-energia_devis" class="tab-pane">
          ...
          <table style="width:100%">
            ...
            ?>
              <tr>
                ...
                <td style="text-align:center">
                  <form method="POST" action="supprimer_devis.php" class="inline" onsubmit="return confirm('Supprimer ce energia_devis ?');">
                    <input type="hidden" name="id" value="<?= (int)$d['id'] ?>">
					<input type="hidden" name="retour" value="clientele.php?client_id=<?= $selected_client_id ?>#pane-energia_devis">
      ...
      <?php else: ?>
        <p class="small-muted">Aucun energia_devis pour ce client.</p>
      <?php endif; ?>
    </div>

    <!-- Pane : Factures -->
    <div id="pane-energia_factures" class="tab-pane">
      <h3>Factures émises</h3>
      <?php if ($energia_factures): ?>
        <div class="info-card" style="padding:0">
          <table style="width:100%">
            <thead><tr><th>Date</th><th>Description</th><th>Total TTC (€)</th><th>PDF</th><th>Actions</th></tr></thead>
            <tbody>
            <?php foreach ($energia_factures as $f):
    ...
    <input type="hidden" name="retour" value="clientele.php?client_id=<?= $selected_client_id ?>#pane-energia_factures">
    ...
    <input type="hidden" name="retour" value="clientele.php?client_id=<?= $selected_client_id ?>#pane-bdc">
    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
    <button class="delete-btn" type="submit">🗑 Supprimer</button>
</form>
...
(function(){
  ...
  const panes = {
    ...
    energia_devis: document.getElementById('pane-energia_devis'),
    energia_factures: document.getElementById('pane-energia_factures'),
...
(function(){
  ...
  const initEmails = <?php
    $seedEmails = $energia_client_emails;
    if ($selected_client_id) {
