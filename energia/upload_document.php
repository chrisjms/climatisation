...
function ensure_document_date_column(PDO $pdo): array {
    $table = 'energia_client_documents';
...
if (!table_exists($pdo, 'energia_client_documents')) {
    @unlink($target_path);
    redirect_err("Table energia_client_documents introuvable.", $client_id);
}

$colInfo = get_column_info($pdo, 'energia_client_documents', 'type');
if ($colInfo && isset($colInfo['Type']) && preg_match('/^enum/i', $colInfo['Type'])) {
    ...
    try {
        ...
        $pdo->exec("ALTER TABLE `energia_client_documents` MODIFY `type` VARCHAR(191) $nullSql$defaultSql");
...
try {
    $cols_table = list_columns($pdo, 'energia_client_documents');
    ...
    $sql = 'INSERT INTO energia_client_documents ('.implode(',', array_map(fn($f)=>"`$f`", $fields)).') VALUES ('.$placeholders.')';
    ...
    if (stripos($msg, 'Incorrect enum value') !== false) {
        @unlink($target_path);
        redirect_err(
            "Impossible d’enregistrer le type libre car la colonne `energia_client_documents.type` est encore ENUM. ".
            "Merci d’exécuter cette commande MySQL puis de réessayer : ".
            "ALTER TABLE energia_client_documents MODIFY `type` VARCHAR(191) NULL;",
            $client_id
        );
    }
