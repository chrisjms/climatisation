...
// Compatible avec vos pages existantes (energia_clients, energia_devis, energia_factures, matériels, energia_brochures)
...
// ───────────────────────── Découverte schéma ─────────────────────────
// Clients
$hasClients = table_exists($pdo, 'energia_clients');
$clientsCols = $hasClients ? list_columns($pdo, 'energia_clients') : [];
$cliDateCol  = $hasClients ? first_col(['date_ajout','created_at','date_creation','inscrit_le'], $clientsCols, null) : null;

// Devis
$hasDevis   = table_exists($pdo, 'energia_devis');
$devisCols  = $hasDevis ? list_columns($pdo, 'energia_devis') : [];
...
// Factures
$hasFact    = table_exists($pdo, 'energia_factures');
$factCols   = $hasFact ? list_columns($pdo, 'energia_factures') : [];
...
$hasBroch   = table_exists($pdo, 'energia_brochures');
$broCols    = $hasBroch ? list_columns($pdo, 'energia_brochures') : [];
$brDate     = $hasBroch ? first_col(['date_ajout','created_at','uploaded_at','date'], $broCols, 'date_ajout') : null;

// Lignes de energia_devis (pour “Top matériels”)
$hasPac     = table_exists($pdo, 'energia_pompes_a_chaleur');
$pacCols    = $hasPac ? list_columns($pdo, 'energia_pompes_a_chaleur') : [];
$pacNameCol = $hasPac ? first_col(['nom','name','label'], $pacCols, 'nom') : null;
$linesFrom  = null; // 'energia_devis_lignes' ou 'energia_devis_items'
if (table_exists($pdo,'energia_devis_lignes')) $linesFrom = 'energia_devis_lignes';
elseif (table_exists($pdo,'energia_devis_items')) $linesFrom = 'energia_devis_items';
...
try {
    if ($hasClients) {
        $nbClients = (int)$pdo->query("SELECT COUNT(*) FROM energia_clients")->fetchColumn();
    }
    if ($hasDevis) {
        $nbDevis = (int)$pdo->query("SELECT COUNT(*) FROM energia_devis")->fetchColumn();
    }
    if ($hasFact) {
        $nbFact = (int)$pdo->query("SELECT COUNT(*) FROM energia_factures")->fetchColumn();

        // CA du mois & de l'année
        $qMonth = $pdo->prepare("SELECT COALESCE(SUM($faTotal),0) FROM energia_factures WHERE DATE($faDate) >= :d1 AND DATE($faDate) <= :d2");
        $qMonth->execute([':d1'=>$firstMonth, ':d2'=>$today]);
        $caMonth = (float)$qMonth->fetchColumn();

        $qYear = $pdo->prepare("SELECT COALESCE(SUM($faTotal),0) FROM energia_factures WHERE DATE($faDate) >= :d1 AND DATE($faDate) <= :d2");
        $qYear->execute([':d1'=>$firstYear, ':d2'=>$today]);
        $caYear = (float)$qYear->fetchColumn();

        // Impayés (plusieurs stratégies)
        if ($faBalance) {
            $impayes = (float)$pdo->query("SELECT COALESCE(SUM(GREATEST($faBalance,0)),0) FROM energia_factures")->fetchColumn();
        } elseif ($faPaidAmt) {
            $impayes = (float)$pdo->query("SELECT COALESCE(SUM(GREATEST($faTotal - COALESCE($faPaidAmt,0),0)),0) FROM energia_factures")->fetchColumn();
        } elseif ($faPaidFlag) {
            // suppose 0 = impayée, 1 = payée
            $impayes = (float)$pdo->query("SELECT COALESCE(SUM($faTotal),0) FROM energia_factures WHERE COALESCE($faPaidFlag,0) IN (0,'0','non','false')")->fetchColumn();
        } else {
    ...
    if ($hasDevis && $faDevisId) {
        $st = $pdo->prepare("
         ...
         LEFT JOIN energia_factures f ON f.$faDevisId = d.id
...
if ($hasDevis) {
    $sql = "
        SELECT d.$dvDate AS date_creation,
               ...
               c.nom, c.prenom
          FROM energia_devis d
          JOIN energia_clients c ON c.id = d.$dvCliId
...
if ($hasFact) {
    $sql = "
        SELECT f.$faDate AS date_creation,
          ...
          JOIN energia_clients c ON c.id = f.$faCliId
...
try {
    ...
    if ($hasClients && $cliDateCol) {
        $st = $pdo->query("SELECT id, $cliDateCol AS dt, nom, prenom FROM energia_clients ORDER BY $cliDateCol DESC, id DESC LIMIT 6");
    ...
    // Devis
    if ($hasDevis) {
        $st = $pdo->query("SELECT id, $dvDate AS dt, description FROM energia_devis ORDER BY $dvDate DESC, id DESC LIMIT 6");
        foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $activite[] = ['date'=>$r['dt'], 'type'=>'Devis', 'label'=>($r['description']??'—'), 'url'=>'devis.php?copy_from_id='.(int)$r['id'].'#form-energia_devis'];
        }
    }
    // Factures
    if ($hasFact) {
        $st = $pdo->query("SELECT id, $faDate AS dt, description FROM energia_factures ORDER BY $faDate DESC, id DESC LIMIT 6");
        foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
    ...
    // Brochures
    if ($hasBroch && $brDate) {
        $st = $pdo->query("SELECT id, $brDate AS dt, COALESCE(titre,'') AS titre FROM energia_brochures ORDER BY $brDate DESC, id DESC LIMIT 6");
        foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
...
if ($linesFrom && $hasPac) {
    try {
        if ($linesFrom === 'energia_devis_lignes') {
            // energia_devis_lignes(pac_id, quantite, prix_unitaire) -> energia_devis(date_creation)
            $sql = "
                SELECT p.id, p.$pacNameCol AS nom,
                  ...
                  JOIN energia_devis d ON d.id = dl.devis_id
            ...
            // energia_devis_items(pac_id, qty, unit_price)
            $sql = "
                SELECT p.id, p.$pacNameCol AS nom,
                       COALESCE(SUM(di.qty),0) AS qty,
                       COALESCE(SUM(di.qty * di.unit_price),0) AS ca_ht
                  FROM energia_devis_items di
                  JOIN energia_devis d ON d.id = di.devis_id
...
<div class="main">
    ...
    <div class="quick" style="margin:12px 0 6px">
        <a class="btn" href="devis.php#form-energia_devis">➕ Nouveau devis</a>
    ...
    <div class="cards">
        ...
        <div class="card">
            ...
            <h3>Derniers energia_devis (TTC)</h3>
            <div class="body">
                ...
                <?php else: ?>
                    <div class="muted">Aucun energia_devis.</div>
                <?php endif; ?>
            </div>

            <h3>Dernières energia_factures (TTC)</h3>
            <div class="body">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                    <div class="kpi" style="padding:10px">
                        ...
                        <div class="value">
                            ...
                            $dv30 = 0; $sumDv30 = 0.0;
                            if ($hasDevis) {
                                $st = $pdo->prepare("SELECT COUNT(*), COALESCE(SUM($dvTotal),0) FROM energia_devis WHERE DATE($dvDate) >= :dmin");
                                $st->execute([':dmin'=>date('Y-m-d', strtotime('-30 days'))]);
