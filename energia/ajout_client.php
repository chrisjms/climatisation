...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Principaux pour la table energia_clients
    ...
    try {
        ...
        $stmt = $pdo->prepare('
            INSERT INTO energia_clients (code_client, nom, prenom, telephone, email, adresse, code_postal, ville, details, date_ajout)
            VALUES (:code_client, :nom, :prenom, :telephone, :email, :adresse, :code_postal, :ville, :details, :date_ajout)
        ');
        $stmt->execute([
            ':nom'         => toNull($nom),
            ':prenom'      => toNull($prenom),
			':code_client' => toNull($code_client),
        ...
        if (!empty($tels_u)) {
            $sqlTel = "
                INSERT INTO energia_client_telephones (client_id, phone, label)
                VALUES (:cid, :ph, :lbl)
        ...
        if (!empty($emails_u)) {
            $sqlMail = "
                INSERT INTO energia_client_emails (client_id, email, label)
                VALUES (:cid, :em, :lbl)
                ON DUPLICATE KEY UPDATE label = VALUES(label)
            ";
                        ...
                        <div class="stack">
							<div>
								...
								<input type="text" id="code_client" name="code_client"
										 value="<?= e($_POST['code_client'] ?? '') ?>">
								<div class="hint">Identifiant interne du client (optionnel).</div>
							</div>
                            <div>
