...
if (!$items || !is_array($items)) {
    exit('Aucune ligne dans le energia_devis.');
}
...
$stmt = $pdo->prepare("SELECT id, prix FROM energia_pompes_a_chaleur WHERE id IN ($in)");
$stmt->execute($ids);
$prixMap = [];
...
try {
    $stmt = $pdo->prepare("INSERT INTO energia_devis (client_nom, total) VALUES (?, ?)");
    $stmt->execute([$client_nom, $total]);
    $devis_id = (int)$pdo->lastInsertId();

    $ins = $pdo->prepare("INSERT INTO energia_devis_items (devis_id, pac_id, qty, unit_price, line_total)
                          VALUES (?, ?, ?, ?, ?)");
    ...
    echo "Erreur lors de l'enregistrement du energia_devis : " . htmlspecialchars($e->getMessage());
}
