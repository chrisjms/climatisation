...
/**
 * Récupère les agrégats (count, sum) pour un tableau donné ('energia_devis' ou 'energia_factures')
 * @return array [labels[], counts[], totals[]]
 */
function getAggregates(PDO $pdo, string $table, string $view): array {
    if (!in_array($table, ['energia_devis', 'energia_factures'], true)) {
...
/**
 * Chiffre d'affaires TTC (energia_factures) :
...
function getTurnover(PDO $pdo): array {
    ...
    // Total global (tout l'historique)
    $stmt = $pdo->query("SELECT COALESCE(SUM(montant_ttc), 0) FROM energia_factures");
...
try {
    ...
    [$labelsDevis, $countDevis, $totalDevis] = getAggregates($pdo, 'energia_devis', $view);
    // Factures (pour les graphes)
    [$labelsFactures, $countFactures, $totalFactures] = getAggregates($pdo, 'energia_factures', $view);
    // Chiffre d'affaires TTC (energia_factures)
...
<div class="main">
    ...
    <div class="grid">
        ...
        <!-- Cartes Graphiques -->
        <div class="card">
            <h3>Nombre de energia_devis (<?= htmlspecialchars($granularityText) ?><?= $view==='all' ? ' — tout l’historique' : '' ?>)</h3>
            <canvas id="chartCountDevis"></canvas>
        </div>

        <div class="card">
            <h3>Nombre de energia_factures (<?= htmlspecialchars($granularityText) ?><?= $view==='all' ? ' — tout l’historique' : '' ?>)</h3>
            <canvas id="chartCountFactures"></canvas>
        </div>

        <div class="card">
            <h3>Montant TTC des energia_devis (<?= htmlspecialchars($granularityText) ?><?= $view==='all' ? ' — tout l’historique' : '' ?>)</h3>
            <canvas id="chartTotalDevis"></canvas>
        </div>

        <div class="card">
            <h3>Montant TTC des energia_factures (<?= htmlspecialchars($granularityText) ?><?= $view==='all' ? ' — tout l’historique' : '' ?>)</h3>
...
makeLineChart('chartTotalDevis',    labelsDevis,    totalDevis,   'Total TTC energia_devis', true);
makeLineChart('chartTotalFactures', labelsFactures, totalFactures,'Total TTC energia_factures', true);
</script>
</body>
</html>
