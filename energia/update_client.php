...
// - Le premier téléphone/email = principal (table energia_clients)
...
$st = $pdo->prepare('SELECT * FROM energia_clients WHERE id = ?');
...
/* ───────── Champs de base (table energia_clients) ───────── */
...
if ($code_client !== '' && mb_strlen($code_client) > 50) { $errors[] = "Code client trop long (max 50 caractères)."; }
...
try {
    ...
    // 1) Update table energia_clients
    $up = $pdo->prepare("
        UPDATE energia_clients
    ...
    ");
    $up->execute([
		':code_client' 	=> ($code_client=== '' ? null:$code_client),
    ...
    if (is_array($phones)) {
        // Rebuild complet
        $pdo->prepare("DELETE FROM energia_client_telephones WHERE client_id = ?")->execute([$client_id]);
        if (!empty($phones)) {
            $ins = $pdo->prepare("INSERT INTO energia_client_telephones (client_id, phone, label) VALUES (:cid, :p, :l)");
        ...
        if ($tel_principal !== '') {
            $ins = $pdo->prepare("INSERT IGNORE INTO energia_client_telephones (client_id, phone, label) VALUES (?, ?, ?)");
    ...
    if (is_array($emails)) {
        // Rebuild complet
        $pdo->prepare("DELETE FROM energia_client_emails WHERE client_id = ?")->execute([$client_id]);
        if (!empty($emails)) {
            $ins = $pdo->prepare("INSERT INTO energia_client_emails (client_id, email, label) VALUES (:cid, :e, :l)");
        ...
        if ($email_principal !== '') {
            $ins = $pdo->prepare("INSERT IGNORE INTO energia_client_emails (client_id, email, label) VALUES (?, ?, ?)");
