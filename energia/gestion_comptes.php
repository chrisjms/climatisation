 ...
 * Migration auto table `energia_users`
 * Debug: ?debug=1
 * ============================================================ */
...
try {
    ...
    /* ---- Schéma energia_users (migration auto) ---- */
    function users_columns(PDO $pdo): array {
        $st = $pdo->query("SHOW COLUMNS FROM `energia_users`");
        $cols = [];
    ...
    function ensure_users_schema(PDO $pdo): void {
        $pdo->exec("
            CREATE TABLE IF NOT EXISTS energia_users (
        ...
        if (!in_array('password_hash', $cols, true)) {
            if (in_array('password', $cols, true)) {
                $pdo->exec("ALTER TABLE `energia_users` CHANGE `password` `password_hash` VARCHAR(255) NOT NULL");
            } else {
                $pdo->exec("ALTER TABLE `energia_users` ADD COLUMN `password_hash` VARCHAR(255) NOT NULL AFTER `username`");
            }
        }
        if (!in_array('role', $cols, true)) {
            $pdo->exec("ALTER TABLE `energia_users` ADD COLUMN `role` ENUM('admin','user') NOT NULL DEFAULT 'user' AFTER `password_hash`");
            if (in_array('is_admin', $cols, true)) {
                $pdo->exec("UPDATE `energia_users` SET `role`='admin' WHERE `is_admin`=1");
            }
        }
        if (!in_array('created_at', $cols, true)) {
            $pdo->exec("ALTER TABLE `energia_users` ADD COLUMN `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `role`");
        }
        if (!in_array('last_login', $cols, true)) {
            $pdo->exec("ALTER TABLE `energia_users` ADD COLUMN `last_login` DATETIME NULL AFTER `created_at`");
        }
        if (!index_exists($pdo, 'energia_users', 'username')) {
            try { $pdo->exec("ALTER TABLE `energia_users` ADD UNIQUE KEY `uniq_users_username` (`username`)"); } catch (Throwable $e) {}
        }
    }
    ...
    $meRole = 'user';
    if ($meUsername !== '') {
        $st = $pdo->prepare("SELECT id, role FROM energia_users WHERE username = :u LIMIT 1");
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'create') {
            ...
            try {
                $st = $pdo->prepare("INSERT INTO energia_users (username, password_hash, role) VALUES (:u, :p, :r)");
        ...
        if ($action === 'passwd') {
            ...
            try {
                $st = $pdo->prepare("SELECT id, username FROM energia_users WHERE id = :id");
                $st->execute([':id'=>$user_id]);
                $u = $st->fetch();
                if (!$u) redirect_msg(null, "Utilisateur introuvable.");

                $hash = password_hash($password, PASSWORD_DEFAULT);
                $up = $pdo->prepare("UPDATE energia_users SET password_hash = :h WHERE id = :id");
        ...
        if ($action === 'delete') {
            ...
            try {
                $st = $pdo->prepare("SELECT id, username, role FROM energia_users WHERE id = :id");
                $st->execute([':id'=>$user_id]);
                $u = $st->fetch();
                if (!$u) redirect_msg(null, "Utilisateur introuvable.");
                ...
                if ($u['role'] === 'admin') {
                    $countAdmins = (int)$pdo->query("SELECT COUNT(*) FROM energia_users WHERE role='admin'")->fetchColumn();
                    if ($countAdmins <= 1) {
                        redirect_msg(null, "Impossible de supprimer le dernier administrateur.");
                    }
                }

                $del = $pdo->prepare("DELETE FROM energia_users WHERE id = :id");
        ...
        if ($action === 'change_role') {
            ...
            try {
                ...
                $up = $pdo->prepare("UPDATE energia_users SET role = :r WHERE id = :id");
    ...
    $params = [];
    $sql = "SELECT id, username, role, created_at, last_login FROM energia_users";
    if ($search !== '') {
      ...
      <form method="post" autocomplete="off" id="form-passwd" class="stack">
        ...
        <div>
          ...
          <select name="user_id" id="user_id" required>
            ...
            <?php foreach ($energia_users as $u): ?>
  ...
  <section id="liste" class="card full" style="margin-top:18px;">
    ...
    <div class="table-wrap">
      <table class="table-sticky">
        ...
        <tbody>
        <?php if (empty($energia_users)): ?>
          <tr><td colspan="5" class="muted">Aucun utilisateur.</td></tr>
        <?php else: foreach ($energia_users as $u): ?>
          <tr>
