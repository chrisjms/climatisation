...
/**
 * Diagnostique les références FK sur energia_clients.id et compte les lignes bloquantes.
...
function diagnose_fk_on_client(PDO $pdo, int $clientId): array {
    ...
    $sql = "
        ...
        WHERE rc.REFERENCED_TABLE_NAME = 'energia_clients'
 ...
 * Ex: ['table'=>'energia_bons_de_commande', 'col_id'=>'id', 'col_client'=>'client_id'|null, 'col_devis'=>'devis_id'|null, 'col_facture'=>'facture_id'|null, 'col_pdf'=>'fichier_pdf'|null]
 */
function find_bdc_mapping(PDO $pdo): ?array {
    $candidates = [
        'energia_bons_de_commande','bons_commandes','bon_de_commande','bon_commandes',
...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, nom, prenom FROM energia_clients WHERE id = ?');
    ...
    // Devis
    $stmt = $pdo->prepare('SELECT id, fichier_pdf FROM energia_devis WHERE client_id = ?');
    $stmt->execute([$client_id]);
    $devis_rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $devis_ids  = array_map(fn($r)=>(int)$r['id'], $devis_rows);

    // Factures
    $stmt = $pdo->prepare('SELECT id, fichier_pdf FROM energia_factures WHERE client_id = ?');
    $stmt->execute([$client_id]);
    $factures_rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $facture_ids   = array_map(fn($r)=>(int)$r['id'], $factures_rows);

    // Documents client (MyISAM)
    $stmt = $pdo->prepare('SELECT id, file_path FROM energia_client_documents WHERE client_id = ?');
    $stmt->execute([$client_id]);
    ...
    // 0) BDC d'abord (pour éviter FK si BDC -> energia_devis/energia_factures/client)
    if ($bdc_map) {
        $t = $bdc_map['table'];
        ...
        // Fallback historique (si table standard energia_bons_de_commande + liaison par devis_id uniquement)
        if ($devis_ids && table_exists($pdo, 'energia_bons_de_commande')) {
            $in = implode(',', array_fill(0, count($devis_ids), '?'));
            $stmt = $pdo->prepare("DELETE FROM energia_bons_de_commande WHERE devis_id IN ($in)");
            $stmt->execute($devis_ids);
        }
    }
    ...
    if ($devis_ids) {
        // energia_devis_items
        $stmt = $pdo->prepare("DELETE di FROM energia_devis_items di JOIN energia_devis d ON d.id=di.devis_id WHERE d.client_id=?");
        try { $stmt->execute([$client_id]); } catch (Throwable $e) {}

        // energia_devis_lignes
        $stmt = $pdo->prepare("DELETE dl FROM energia_devis_lignes dl JOIN energia_devis d ON d.id=dl.devis_id WHERE d.client_id=?");
        try { $stmt->execute([$client_id]); } catch (Throwable $e) {}

        // energia_devis_paiements
        $stmt = $pdo->prepare("DELETE dp FROM energia_devis_paiements dp JOIN energia_devis d ON d.id=dp.devis_id WHERE d.client_id=?");
        try { $stmt->execute([$client_id]); } catch (Throwable $e) {}
    }

    // 2) Factures du client (si pas de FK cascade)
    if ($facture_ids) {
        // Si vous avez d’autres tables liées aux energia_factures, supprimez-les ici en amont.
        $stmt = $pdo->prepare('DELETE FROM energia_factures WHERE client_id = ?');
        $stmt->execute([$client_id]);
    }

    // 3) Devis du client
    $stmt = $pdo->prepare('DELETE FROM energia_devis WHERE client_id = ?');
    $stmt->execute([$client_id]);

    // 4) Emails / Téléphones (par sécurité même si CASCADE existe)
    $stmt = $pdo->prepare('DELETE FROM energia_client_emails WHERE client_id = ?');
    $stmt->execute([$client_id]);
    $stmt = $pdo->prepare('DELETE FROM energia_client_telephones WHERE client_id = ?');
    $stmt->execute([$client_id]);

    // 5) Client
    $stmt = $pdo->prepare('DELETE FROM energia_clients WHERE id = ?');
    $stmt->execute([$client_id]);
    ...
    // energia_client_documents (MyISAM) : lignes + fichiers
    if ($doc_rows) {
        $stmt = $pdo->prepare('DELETE FROM energia_client_documents WHERE client_id = ?');
        try { $stmt->execute([$client_id]); } catch (Throwable $e) {
            log_msg("Suppression energia_client_documents échouée mais ignorée: ".$e->getMessage());
        }
    }
