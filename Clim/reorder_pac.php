...
try {
    ...
    foreach ($data['orders'] as $catKey => $ids) {
        ...
        foreach ($ids as $id) {
            if ($isNull) {
                $stmt = $pdo->prepare('UPDATE pompes_a_chaleur SET position = ? WHERE id = ? AND category_id IS NULL');
                $stmt->execute([$position, $id]);
            } else {
                $stmt = $pdo->prepare('UPDATE pompes_a_chaleur SET position = ? WHERE id = ? AND category_id = ?');
