<?php
// brochures.php — gestion des brochures (upload + listing + édition description)
require 'auth.php';
require 'config.php';
...
if ($action === 'delete' && $id) {
    ...
    $stmt = $pdo->prepare("SELECT fichier_path FROM brochures WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($action === 'upload' || $action === 'list')) {
    ...
    if (!isset($_FILES['brochure']) || $_FILES['brochure']['error'] === UPLOAD_ERR_NO_FILE) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            $destDir = __DIR__ . '/uploads/brochures';
            ...
            if (!$errors) {
                ...
                $relPath  = 'uploads/brochures/' . $filename; // pour la base (chemin relatif web)
                ...
                if (!move_uploaded_file($file['tmp_name'], $destPath)) {
                    ...
                    // Enregistrement BDD
                    $stmt = $pdo->prepare("
                        INSERT INTO brochures (titre, description, fichier_path, mime_type, taille_octets, date_ajout)
